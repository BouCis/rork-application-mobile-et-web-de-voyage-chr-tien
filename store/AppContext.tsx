...
export const [AppProvider, useApp] = createContextHook(() => {
  ...
  const saveUser = useCallback(async (userData: User) => {
    try {
      ...
      setUser(userData);
      console.log('[AppContext] User saved:', userData.id);
    } catch (error) {
  ...
  const addTrip = useCallback(async (trip: Trip) => {
    try {
      ...
      console.log('[AppContext] Trip added:', trip.id);
    } catch (error) {
  ...
  const updateTrip = useCallback(async (tripId: string, updates: Partial<Trip>) => {
    try {
      ...
      await AsyncStorage.setItem(STORAGE_KEYS.TRIPS, JSON.stringify(updatedTrips));
      setTrips(updatedTrips);
      console.log('[AppContext] Trip updated:', tripId);
    } catch (error) {
  ...
  const deleteTrip = useCallback(async (tripId: string) => {
    try {
      ...
      await AsyncStorage.setItem(STORAGE_KEYS.TRIPS, JSON.stringify(updatedTrips));
      setTrips(updatedTrips);
      console.log('[AppContext] Trip deleted:', tripId);
    } catch (error) {
