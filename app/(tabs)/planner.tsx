...
export default function PlannerScreen() {
  ...
  const shortcuts = useMemo(() => ['Weekend', 'Plage', 'Montagne', "Cite d'art", 'Road-trip'], []);
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    brand: { color: colors.text, fontSize: 18, fontWeight: '700' as const, letterSpacing: 0.4 },
    avatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center' },

    promptCard: { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 18, padding: 16, gap: 10 },
    promptText: { color: colors.text, fontSize: 16, fontWeight: '700' as const, lineHeight: 22 },
    chipRow: { flexDirection: 'row', flexWrap: 'wrap' as const, gap: 8 },
    chip: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, backgroundColor: colors.backgroundSecondary, borderWidth: 1, borderColor: colors.border },
    chipActive: { backgroundColor: colors.backgroundLight, borderColor: colors.borderLight },
    chipText: { color: colors.textSecondary, fontSize: 12, fontWeight: '600' as const },

    searchBar: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 14, paddingVertical: 12, borderRadius: 16, backgroundColor: colors.backgroundSecondary, borderWidth: 1, borderColor: colors.border },
    searchInput: { flex: 1, fontSize: 15, color: colors.text },
    shortcutsRow: { flexDirection: 'row', gap: 8, marginTop: 10 },
    shortcut: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, borderWidth: 1, borderColor: colors.border },
    shortcutText: { color: colors.textSecondary, fontSize: 12 },
    resultsWrap: { position: 'absolute', top: 58, left: 0, right: 0, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, maxHeight: 280, overflow: 'hidden' },
    resultItem: { flexDirection: 'row', alignItems: 'center', padding: 12, gap: 10, borderBottomWidth: 1, borderBottomColor: colors.border },
    resultName: { color: colors.text, fontSize: 15, fontWeight: '600' as const },
    ...
    deckCard: { width: width * 0.8, borderRadius: 18, overflow: 'hidden', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border },
    deckImage: { width: '100%', height: 200 },
    deckOverlay: { position: 'absolute', left: 0, right: 0, bottom: 0, padding: 12, backgroundColor: isDark ? 'rgba(15, 23, 42, 0.85)' : 'rgba(255, 255, 255, 0.9)' },
    deckTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    titleText: { color: colors.text, fontSize: 18, fontWeight: '800' as const },
    subText: { color: colors.textSecondary, fontSize: 12, marginTop: 2 },
    pricePill: { backgroundColor: isDark ? colors.backgroundSecondary : colors.backgroundTertiary, borderColor: colors.borderLight, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 },
    priceText: { color: colors.secondary, fontSize: 12, fontWeight: '800' as const },
    ctaBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 },
    ctaBtn: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, backgroundColor: colors.backgroundLight, borderWidth: 1, borderColor: colors.borderLight },
    ctaLabel: { color: colors.primary, fontSize: 12, fontWeight: '700' as const },
    ...
    roundLabel: { color: colors.text, fontSize: 12, marginTop: 6 },
    roundMeta: { color: colors.textSecondary, fontSize: 11 },

    grid: { flexDirection: 'row', flexWrap: 'wrap' as const, gap: 10 },
    wishTile: { width: (width - 20 * 2 - 10) / 2, height: 120, borderRadius: 14, overflow: 'hidden', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border },
    wishBadge: { position: 'absolute', top: 8, left: 8, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, backgroundColor: isDark ? colors.backgroundSecondary : colors.backgroundTertiary, borderWidth: 1, borderColor: colors.borderLight },
    wishBadgeText: { color: colors.secondary, fontSize: 11, fontWeight: '700' as const },

    timelineCard: { backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1, borderColor: colors.border, padding: 14 },
    timelineItem: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8 },
    dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.primary },
    line: { position: 'absolute', left: 3.5, top: 0, bottom: 0, width: 1, backgroundColor: colors.border },

    duoRow: { flexDirection: 'row', gap: 12 },
    miniCard: { flex: 1, backgroundColor: colors.surface, borderRadius: 16, borderWidth: 1, borderColor: colors.border, padding: 14 },
    miniTitle: { color: colors.text, fontSize: 14, fontWeight: '700' as const, marginBottom: 8 },
    ...
    iaChip: { paddingHorizontal: 12, paddingVertical: 10, borderRadius: 12, backgroundColor: colors.backgroundSecondary, borderWidth: 1, borderColor: colors.border },
    iaText: { color: colors.text, fontSize: 12 },
  }), [insets.top, colors, isDark]);

  return (
    <View style={styles.container} testID="home-root">
      <Animated.View style={[styles.skyWrap, { transform: [{ translateY: skyTranslate }] }]}>        
        <LinearGradient 
          colors={isDark ? [colors.background, colors.backgroundSecondary] : [colors.background, colors.backgroundLight]} 
      ...
      >
        <View style={styles.header}>
          <View style={styles.headerRow}>
            <View style={styles.badge}>
              {isEvening ? <Moon color={colors.textSecondary} size={18} /> : isMorning ? <Sun color={colors.textSecondary} size={18} /> : <Compass color={colors.textSecondary} size={18} />}
        ...
        <View style={styles.section}>
          <View style={styles.sectionTitleRow}><Text style={styles.sectionTitle}>Wishlist</Text><Heart color={colors.primary} size={16} /></View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionTitleRow}><Text style={styles.sectionTitle}>Mes voyages</Text><Calendar color={colors.textSecondary} size={16} /></View>
          {trips.length === 0 ? (
              ...
              <View style={styles.timelineCard}>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                  {['City-break 3j < 300€', 'Micro‑aventure 2j', 'Nomad 1 mois'].map((p) => (
                    <View key={p} style={[styles.chip, { backgroundColor: colors.backgroundLight, borderColor: colors.borderLight }]}><Text style={[styles.chipText, { color: colors.primary }]}>{p}</Text></View>
                  ))}
                </View>
              </View>
            </TouchableOpacity>
            ...
            <View style={[styles.timelineCard, { position: 'relative' }]}>
              ...
              {trips.slice(0, 3).map((t) => (
                <View key={t.id} style={styles.timelineItem}>
                  ...
                  <Text style={{ color: colors.text, fontSize: 13, fontWeight: '700' as const }}>{t.destination}</Text>
        ...
        <View style={styles.section}>
          <View style={styles.duoRow}>
            <View style={styles.miniCard}>
              ...
              <View style={styles.checkRow}><Text style={styles.checkLabel}>Passeport 6 mois</Text><Text style={[styles.checkLabel, { color: colors.primary }]}>À faire</Text></View>
              <View style={styles.checkRow}><Text style={styles.checkLabel}>Demande eVisa</Text><Text style={[styles.checkLabel, { color: colors.textSecondary }]}>Lien officiel</Text></View>
            </View>
            <View style={styles.miniCard}>
              <Text style={styles.miniTitle}>Santé & vaccins</Text>
              <View style={styles.checkRow}><Text style={styles.checkLabel}>Fièvre jaune</Text><Text style={[styles.checkLabel, { color: colors.primary }]}>Rappel</Text></View>
              <View style={styles.checkRow}><Text style={styles.checkLabel}>Carnet vaccination</Text><Text style={[styles.checkLabel, { color: colors.textSecondary }]}>Ok</Text></View>
            </View>
          </View>
        </View>

        <View style={styles.section}>
          <View style={styles.sectionTitleRow}><Text style={styles.sectionTitle}>Explorations IA</Text><Sparkles color={colors.primary} size={16} /></View>
