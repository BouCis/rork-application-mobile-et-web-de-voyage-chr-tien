...
const raw: Record<string, { name: string; cities: ({ name: string; lat: number; lng: number } | string)[] }> = require('./countries-cities.json');
...
const countriesFromJson: Country[] = Object.entries(raw).map(([iso2, v]) => ({
  ...
  cities: v.cities.map((c) => {
    if (typeof c === 'string') {
      return { name: c } as City;
    }
    return { name: c.name, lat: c.lat, lng: c.lng } as City;
  }),
}));
